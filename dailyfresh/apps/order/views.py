...
# /order/place
class OrderPlaceView(LoginRequestedMixin, View):
    '''提交订单页面显示'''
...
# /order/commit
# 前端传递的参数:地址id(addr_id) 支付方式(pay_method) 用户需要购买的商品id字符串(sku_ids)
class OrderCommitView1(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            # print('addr_id的类型:', type(addr_id)) 字符串类型
        ...
        try:
            ...
            for sku_id in sku_ids:
                # 获取商品信息
                try:
                    # 加悲观锁查询,若用户1未释放锁，用户2执行到此也会堵塞．
                    sku = GoodsSKU.objects.select_for_update().get(id=sku_id)
                    # print('sku_id的类型:', type(sku_id)) 字符串类型
                except:
                    # 商品不存在
                ...
                for i in range(3):
                    ...
                    if res == 0:
                        if i == 2:
                            # 尝试第三次
                            transaction.savepoint_rollback(save_id)
                            return JsonResponse({'res': 7, 'errmsg': '下单失败２'})
                        continue
                    # todo: 向df_order_goods表中加入一条记录
                    OrderGoods.objects.create(order=order,
                                              sku=sku,
...
class OrderCommentView(LoginRequestedMixin, View):
    """订单评论"""
